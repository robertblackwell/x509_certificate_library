...
///
bool check_host_dns_altname(std::string host, std::string dns_name)
{
...
bool Cert_checkHostInAltnameString(std::string host, std::string altNames)
{
    ...
    for(int i = 0; i < dns_patterns.size(); i++) {
        if(check_host_dns_altname(host, dns_patterns[i])){
            return true;
        }
    }
    return false;
...
{
    boost::optional<std::string> alt_names_opt = Cert_GetSubjectAlternativeNamesAsString(cert_x509);
    if (!alt_names_opt) {
        return false;
    }
    bool r = Cert_checkHostInAltnameString(host, alt_names_opt.get());
    return r;
}
} // namespace
} // namespace
